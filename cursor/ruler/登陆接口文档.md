...
由于遇到401认证问题，这里提供完整的Spring Security配置：

## SecurityConfig.java 完整配置
...
1. **配置类注解**：确保有`@Configuration`和`@EnableWebSecurity`
2. **Bean注册**：SecurityFilterChain必须是@Bean
3. **路径匹配**：检查requestMatchers中的路径是否正确
4. **配置顺序**：SecurityConfig应该在AuthConfig之前或合并到一个配置中
...
public class SecurityConfig {
    ...
    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        log.info("正在配置Spring Security...");
        
        http.authorizeHttpRequests(auth -> {
            log.info("配置公开接口: /auth/login, /auth/register 等");
...
public class AuthConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(authInterceptor)
                .addPathPatterns("/auth/userInfo", "/auth/logout")
                .excludePathPatterns(
                        "/auth/login",
                        "/auth/register",
                        "/auth/sendVerificationCode", 
